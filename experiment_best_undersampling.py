...
def compute(clf_name, clf, chunk_size, n_chunks, experiment_name, n_drifts, stream_name):
    ...
    try:
        ...
        if os.path.exists("results/raw_conf/%s/cluster_setup/%s/%s.csv" % (experiment_name, stream_name.split("/")[-1], clf_name)):
            return
...
for base_estimator, experiment_name in zip(base_estimators, experiment_names):
    ...
    methods = {
               "DSO-CC": DSO(undersampling=under_sampling.ClusterCentroids()),
               "DSO-CNN": DSO(undersampling=under_sampling.CondensedNearestNeighbour()),
               "DSO-ENN": DSO(undersampling=under_sampling.EditedNearestNeighbours()),
               "DSO-RENN": DSO(undersampling=under_sampling.RepeatedEditedNearestNeighbours()),
               "DSO-AKNN": DSO(undersampling=under_sampling.AllKNN()),
               "DSO-IHS": DSO(undersampling=under_sampling.InstanceHardnessThreshold()),
               "DSO-NM": DSO(undersampling=under_sampling.NearMiss()),
               "DSO-NCR": DSO(undersampling=under_sampling.NeighbourhoodCleaningRule()),
               "DSO-OSS": DSO(undersampling=under_sampling.OneSidedSelection()),
               "DSO-RUS": DSO(undersampling=under_sampling.RandomUnderSampler()),
               "DSO-TL": DSO(undersampling=under_sampling.TomekLinks()),
              }
