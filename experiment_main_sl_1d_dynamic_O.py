...
def compute(clf_name, clf, drift, noise, imbalance_ratio, random_state, experiment_name, concept_kwargs):
    ...
    try:
        ...
        stream_name = "stream_sldi_%dd_%s_%03dk_f%02d_b%02d_n%02d_rs%03d" % (n_drifts, drift[0], stream_size, concept_kwargs["n_features"], imbalance_ratio[2]*100, noise*100, random_state)

        if os.path.exists("results/raw_conf/%s/sl_%dd/%s/%s/%s.csv" % (experiment_name, n_drifts, drift, stream_name, clf_name)):
            return
        ...
        filename = "results/raw_conf/%s/sl_%dd_dyn/%s/%s/%s.csv" % (experiment_name, n_drifts, drift, stream_name, clf_name)
        if not os.path.exists("results/raw_conf/%s/sl_%dd_dyn/%s/%s/" % (experiment_name, n_drifts, drift, stream_name)):
            os.makedirs("results/raw_conf/%s/sl_%dd_dyn/%s/%s/" % (experiment_name, n_drifts, drift, stream_name))
