...
def compute(clf_name, clf, chunk_size, n_chunks, experiment_name, n_drifts, stream_name):
    ...
    try:
        ...
        # if os.path.exists("results/raw_conf/%s/%s/%s/%s.csv" % (experiment_name, n_drifts, drift, stream_name.split("/")[-1], clf_name)):
        #     return
...
for base_estimator, experiment_name in zip(base_estimators, experiment_names):
    ...
    methods = {
                "DSO-SHS": DSO(cluster_metric=silhouette_score),
                "DSO-CHS": DSO(cluster_metric=calinski_harabasz_score),
                "DSO-DBS": DSO(cluster_metric=davies_bouldin_score),
              }
